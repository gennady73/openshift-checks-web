...
def scheduler_action():
    ...
    if action_param == "scheduler_stop":
        ...
        if response.status_code == 200 or response.status_code == 204:
            ...
            print(f"Failed stopping scheduler!")
        ...
        if response.status_code == 200 or response.status_code == 204:
            ...
            print(f"Failed starting scheduler!")
    else:
        print(f"Invalid parameter: {action_param}")
...
def scheduler_job(job_id):
    ...
    try:
        ...
        for job_log in job_logs:
            print(f"Get job log:{job_log}")

    except Exception as ex:
        print(f'Failed to get job log: {ex}')
...
def scheduler_start_job(job_id):
    print(f">>> scheduler_start_job : {job_id}")
...
def scheduler_stop_job(job_id):
    print(f">>> scheduler_stop_job : {job_id}")
...
def scheduler_jobs_old():
    ...
    # print(f"url_jobs    : {url_for('scheduler.get_jobs')}")
    # print(f"host prefix : {request.host_url}")
    #
...
def scheduler_jobs():
    ...
    if request.method == 'POST':
        ...
        if action_param is not None and 'delete' == action_param:
            job_data = get_job_data(request.form, field_filter=['id', 'name', 'jobstore'])
            try:
...
def event_listener(event):
    ...
    if event.code == EVENT_JOB_ERROR:
        ...
        logging.getLogger('apscheduler').info(f'The job "{event.job_id}" worked :)')
...
def update_scheduler_global_state(global_state: dict, jobstore: str, job_id=None):
    ...
    try:
        ...
        for job_log in job_logs:
            print(f"log:{job_log}")  # logging.getLogger('apscheduler').debug
            ###
            #
            # job_log = JobLog(job_id=event.job_id, timestamp=event.scheduled_run_time)
            # job_log.event = event
            ...
            logging.getLogger('apscheduler').info(f'The job log "{event.job_id}" initializing.')
